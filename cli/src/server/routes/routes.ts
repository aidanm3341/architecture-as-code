...
export class CLIServerRoutes {
    ...
    constructor(schemaDirectoryPath: string, schemaDirectory: SchemaDirectory, debug: boolean = false) {
        this.router = Router();
        const validateRoute = this.router.use(VALIDATE_ROUTE_PATH, this.router);
        new ValidationRouter(validateRoute, schemaDirectoryPath, schemaDirectory, debug);
